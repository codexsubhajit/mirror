...
//                                val payload = mapOf(
//                                    "employeeid" to empIdInt,
//                                    "attendance_datetime" to formattedDate,
//                                    "mirror_image" to url
//                                )
                                //val outgoingJson = com.google.gson.Gson().toJson(payload)
                               // Log.d("AttendanceSyncUtil", "Outgoing JSON: $outgoingJson")
//                                val apiSuccess = FaceAttendanceApi.sendFaceAttendance(
//                                    token = token,
//                                    empId = log.empId,
//                                    attendanceDatetime = formattedDate,
//                                    mirrorImageUrl = url
//                                )
                                //Log.d("AttendanceSyncUtil", "Face attendance API for empId=${log.empId}")
                            } catch (e: Exception) {
        ...
        val batchPayload = result.filter { it.second != null }
            .map { (log, url) ->
                ...
                mapOf(
                    ...
                    "target_image" to (url as Any),
                    "type" to (3 as Any),
                    "_sort_recordTime" to recordTime // for sorting only, not sent to API
                )
            }
            .sortedBy { it["_sort_recordTime"] as String }
            .map { it - "_sort_recordTime" } // remove the sort helper before sending
